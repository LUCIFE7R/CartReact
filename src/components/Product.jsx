  ...
  return (
    <div className="flex flex-col items-center justify-between hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 rounded-xl outline">
      ...
      <div className="flex justify-between gap-12 items-center w-full mt-5">
        <div>
          <p className="text-green-600 font">${post.price}</p>
        </div>
        {
          cart.some((p)=>p.id==post.id)?
          (<button className="text-gray-700 bordeer-2 border-gray-700 rounded-full  font-semibold
          text-[12px] px-3 p-1 uppercase
          hover:bg-gray-700 
          ...
          text-[12px] p-1 px-3 uppercase" onClick={addToCart}>Add to Cart</button>)
        }
      </div>
    </div>
  );
